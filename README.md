...
2. `lowerBand` function, the implementation.
    ...
    1. left指向的是满足条件的第一个元素的下标
    2. right可以代表满足改条件的数组的长度
...
2. 在有正负数的情况下，可以使用set来进行遍历，如果数据量足够小，那么可以使用一个boolean数组来确定
    ...
    1. 正负数的情况下， 可以先遍历正数，然后再找相应的负数

3. LeeCode 3623 题的一些注意事项

    1. 解题思路：
        1. 这道题明显不会去要计算每个点能够产生的平行线来计算，一定要根据题目所给的内容来取巧
        2. 找出在同一水平线上的点是解题的关键，因为找到了点，就可以直接计算在同一水平线上，能够有多少的线

4. LeetCode 3805

    1. 解题思路：

        1. 这道题很明显和3623是一个思路，唯一不同的就是怎么处理字符串
